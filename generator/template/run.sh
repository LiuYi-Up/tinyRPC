...
echo "Run tinyrpc rpc project, name: ${PROJECT_NAME}, path: ${PROJECT_BIN_FILE}"
...
then
  echo "Run tinyrpc rpc server eror, file: ${PROJECT_BIN_FILE} not exist, please check file"
  exit -1
fi
...
then
  echo "Run tinyrpc rpc error, file: ${PROJECT_CONF_FILE} not exist, please check config file"
  exit -1
fi
...
echo "Start tinyrpc rpc server ${PROJECT_CONF_FILE} succ"
